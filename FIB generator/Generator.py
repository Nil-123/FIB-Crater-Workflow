...
script_version='V3'
comment='Improment from FIB35 AFM results'
...
size_um=15 #length in um of the edge of the square milling area
RoC_um=18 #desired radius of curviture in um
H_um=0.3 #desired depth of the crater, measured from the plateau after initial milling step
H_observed=0.27 #Fitted from previous AFM session
...
path="S:\\Topics\\Open_cavity\\Samples\\FIB\\streamfiles_database\\"
set_current=26*10**(-12) #in Amps 
...
pixel_order=0 #0- serpentine, 1- random
ordering=['serpentine', 'random']
correction_method_names=['none', 'pick_depth_scaling_factor(set_current)', 'set/get_current correction']
iterator_file_path="S:\\Topics\\Open_cavity\\Samples\\FIB\\streamfiles_database\\magicFIBiterator.txt"
...
def pick_depth_scaling_factor(curr, correction_method):
    if correction_method==1:
        if curr== 9*10**(-12):
            return 0.83*(H_um/H_observed)   # 0.5976/1.2   =0.3/0.502 <- this comes form test milling with set/get current correction, and the obtained depth, 1.2 additional correction after second test milling
        elif curr==26*10**(-12):
            return 0.7879 #=26/33
        elif curr==46*10**(-12):
            return 0.566
...
# fig = go.Figure(data=[go.Surface(z=pixels, x=x, y=y)])
# fig.update_layout(
#     title='3D Visualization of Helical Surface',
#     scene=dict(
#         xaxis_title='X (um)',
#         yaxis_title='Y (um)',
...
database=open(path+'streamfile_database.csv', 'a')
#milled label	file name	date	H (um)	RoC (um)	sigma (um)	max_dwell_time (*0.1 us)	Magnification	set_current (A)	screen filling factor	dose (um^3/nC)	pixel ordering	desired milling area size (um)	milling area size (um)	resolution (px)	spacing (um)	spacing in FIB coordinates (px)	H correction method	get_current (A)	pick_depth_scaling_factor	# of passses one_pass_milling_time (ms) total_milling_time (s)	script version	comments
saved_data=[label_text, streamfile_name, date,  H_um, RoC_um, sigma_um, max_dwell_time, M, set_current, screen_filling_factor, dose, ordering[pixel_order], size_um, expected_size_um, pattern_resolution, spacing*a, spacing, [height_correction_method, correction_method_names[height_correction_method]], get_current, H_factor, number_of_passes, one_pass_milling_time, total_milling_time, script_version, comment]
saved_string = ';'.join(str(i) for i in saved_data)+'\n'
database.write(saved_string)
database.close()
...
# fig.update_layout(
#     title=f"Crater Fit | ROC_x = {ROC_x:.2f} µm, ROC_y = {ROC_y:.2f} µm",
#     scene=dict(
#         xaxis_title='X (µm)',
#         yaxis_title='Y (µm)',
