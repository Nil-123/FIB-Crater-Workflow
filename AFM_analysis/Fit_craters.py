...
def analyze_crater(
    ...
    if show_plot==True:
        ...
        plt.xlabel("X (µm)")
        plt.ylabel("Y (µm)")
        plt.title("AFM data for fitting")
        plt.show()
    
    if save_data==True:
        df = pd.DataFrame(z_sub, index=y_sub, columns=x_sub)
        df.to_csv(save_data_directory/'afm_grid.csv')
    ...
    print("----Results from 2D Gaussian fitting----")
    ...
    if show_plot==True:
        ...
        plt.legend()
        #plt.grid(True)
        plt.title("Gaussian fit for crossections")
        #plt.show()
    ...
    if save_data==True:
        ...
        new_df.to_csv(save_data_directory/"H-V_plots.csv", index=False)
    ...
    if show_plot==True:
        ...
        plt.title("Surface fit Residuals")
        plt.xlabel("X (µm)")
        plt.ylabel("Y (µm)")
        plt.show()
    ...
    if save_data==True:
        ...
        df.to_csv(save_data_directory/'afm_grid_residuals.csv')
    ...
    if show_plot==True:
        ...
        fig.update_layout(
            ...
            title=f"AFM data with gaussian Fit",
            width=800,
            height=600
        )
        ...
        fig = go.Figure(data=[go.Surface(z=residuals, x=x_sub, y=y_sub, colorscale='Viridis')])
        fig.update_layout(
            title=f'Residuals from Gaussian Fit RMSE {rmse:.3f} nm ',
            scene=dict(
                xaxis_title='X (um)',
                yaxis_title='Y (um)',
